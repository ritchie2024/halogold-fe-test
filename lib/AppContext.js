...
import {
  ...
  VERIFICATION_TRAIL,
  PERSONAL_BATCH,
  YIELD_ACTIVE_POSITION,
  YIELD_TENOR_OPTIONS,
  GIFT_THEMES,
  DUMMY_CONTACTS,
} from "./dummyData";

const STORAGE_KEY = "halogold-state-v3";
...
export function AppProvider({ children }) {
  ...
  function printFisik(gramToPrint, mintingFee, shippingMethod) {
    ...
    const newTransaction = {
      ...
      statusNote: shippingMethod?.id === "gerai" ? "siap diambil \u2713" : "diproses \u00b7 resi terasuransi",
    };
  ...
  function placeYield(gramToPlace, tenorOption) {
    if (gramToPlace > balanceGram) {
      throw new Error("Saldo emas tidak mencukupi.");
    }
    ...
    const newTransaction = {
      ...
      type: "yield",
      gram: gramToPlace,
      amount: gramToPlace * GOLD_PRICE_PER_GRAM,
      date: new Date().toISOString(),
      status: "berhasil",
      method: `Tempatkan Emas Berimbal \u00b7 tenor ${tenorOption.months} bulan`,
      statusNote: "terkunci hingga jatuh tempo",
    };

    setBalanceGram((prev) => prev - gramToPlace);
    setTransactions((prev) => [newTransaction, ...prev]);

    return newTransaction;
  }

  function sendGold(gramToSend, recipientPhone, mode, note) {
    if (gramToSend > balanceGram) {
      throw new Error("Saldo emas tidak mencukupi.");
    }

    const recipientName = Object.entries(DUMMY_CONTACTS).find(([prefix]) =>
      recipientPhone.startsWith(prefix)
    )?.[1];

    const newTransaction = {
      id: `TRX-${Math.floor(10000 + Math.random() * 89999)}`,
      type: mode === "kado" ? "kado" : "kirim",
      gram: gramToSend,
      amount: gramToSend * GOLD_PRICE_PER_GRAM,
      date: new Date().toISOString(),
      status: "berhasil",
      method: recipientName
        ? `${mode === "kado" ? "Kado untuk" : "Transfer ke"} ${recipientName}`
        : `${mode === "kado" ? "Kado ke" : "Transfer ke"} ${recipientPhone}${
            recipientName ? "" : " (tautan klaim 30 hari)"
          }`,
      statusNote: recipientName ? "diklaim \u2713" : "menunggu klaim",
    };

    setBalanceGram((prev) => prev - gramToSend);
    setTransactions((prev) => [newTransaction, ...prev]);

    return { ...newTransaction, recipientName, claimLink: !recipientName };
  }
  ...
  const value = {
    ...
    yieldActivePosition: YIELD_ACTIVE_POSITION,
    yieldTenorOptions: YIELD_TENOR_OPTIONS,
    giftThemes: GIFT_THEMES,
    dummyContacts: DUMMY_CONTACTS,
    balanceGram,
    tokenBalance,
    transactions,
    theme,
